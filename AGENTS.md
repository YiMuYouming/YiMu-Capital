...
```
docs/audit/2026-05-26-升级改造执行审查与任务清单.md    ← 总体任务地图
docs/audit/2026-05-26-给{米名}-{Gate编号}-{任务名}.md  ← 各米执行指令
docs/audit/2026-05-26-{Gate编号}-{任务名}.md            ← 实施计划（执行用）
```

命名规律：
- `给洋米` = 给洋米的执行指令（直接复制发给洋米）
- `给黑米` = 给黑米的执行指令（直接复制发给黑米）
- `升级改造执行审查与任务清单` = 总体地图（欧米用）

每次开工只需读总体地图，看「第五节 实施规则与分工」的 Gate 批次表，找到对应米和 Gate，定位到第六节及之后的具体任务章节。
...
任务：HM-G0A / HM-G0B / HM-G0C（与任务文档中一致）

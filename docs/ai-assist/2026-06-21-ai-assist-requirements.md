...
AI Assist 可以生成票据、解释规则、提示风险、排序待办，但不能自动下单、自动确认成交或自动跳过人工复核。弈沐自然语言明确授权后的代操作不属于“自动确认”：对象和成交字段完整时可执行 `preview → confirm → 四项回读`，无需重复确认。
...
- 不允许 AI 自行或定时自动确认成交；允许弈沐自然语言明确授权后由 Agent 代执行正式 `preview → confirm → 四项回读`。
- 不允许在健康阻断时生成买入可执行结论。

...
const WidgetRegistry = (function() {
  ...
  var widgets = [
    ...
    { id:'W03', type:'position-calc', title:'三层仓位计',   category:'decision', tier:'tick',defaultSize:{w:6,h:6},  dataPaths:['style.总仓位上限','style.连板占比','style.趋势占比','style.实际执行.连板实际','style.实际执行.趋势实际','style.实际执行.首笔上限','risk.熔断触发','risk.连亏天数','positions','live_quotes','pnl_live','rule_state'], priority:'P0' },
    { id:'W06', type:'auction-5d',    title:'竞价5维面板',  category:'decision', tier:'manual', defaultSize:{w:12,h:6},  dataPaths:['auction_snapshot','sentiment.情绪值','sentiment.昨日涨停收益','sentiment.昨日炸板收益','sentiment.连板收益','sentiment.连板风险值','sentiment.赚钱效应','sentiment.晋级率','sentiment.最高板','sentiment.封板率','style.一进二晋级率','style.二进三晋级率','style.三进四晋级率','style.趋势走强板块数'], priority:'P1' },
    { id:'W07', type:'climax-guard',  title:'高潮保护',     category:'decision', tier:'manual', defaultSize:{w:3,h:2},  dataPaths:['sentiment.竞价情绪值','rule_state'], priority:'P1' },
    { id:'W08', type:'w1-check',      title:'W1早盘确认',   category:'decision', tier:'tick',   defaultSize:{w:4,h:6},  dataPaths:['decision.早盘','lianban_pool','live_quotes','rule_state'], priority:'P1' },
    { id:'W09', type:'w2-check',      title:'W2实时观察',   category:'decision',tier:'tick',   defaultSize:{w:10,h:7},  dataPaths:['sentiment_nodes','sentiment.情绪值','sentiment.昨日涨停收益','trend_pool','live_quotes','live_index','rule_state'], priority:'P1' },
    // 数据类
    ...
    // 风控类
    { id:'W14', type:'risk-panel',    title:'账户风控',     category:'risk',     tier:'tick', defaultSize:{w:4,h:5}, dataPaths:['risk.单日熔断线','risk.周累计回撤','risk.月累计回撤','risk.连亏天数','positions','live_quotes','pnl_live','rule_state'], priority:'P0' },
    { id:'W15', type:'positions',     title:'持仓快照',       category:'risk',     tier:'tick',defaultSize:{w:12,h:6}, dataPaths:['positions','decision.今日操作','live_quotes','pnl_live'], priority:'P0' },
    // 工具类
    { id:'W16', type:'input-panel',   title:'报数面板',     category:'tool',     tier:'manual', defaultSize:{w:12,h:2}, dataPaths:[], priority:'P1' },
    // 决策类（v2.1 新增）
    { id:'W17', type:'today-ops',     title:'今日操作',     category:'decision', tier:'manual', defaultSize:{w:6,h:3}, dataPaths:['decision.今日操作','pnl_live'], priority:'P1' },
    ...
    { id:'W19', type:'midday-review',title:'午盘复核',     category:'decision', tier:'manual', defaultSize:{w:4,h:4}, dataPaths:['decision.盘中','rule_state'], priority:'P1' },
    { id:'W20', type:'llm-monitor',  title:'研判摘要',     category:'decision', tier:'manual', defaultSize:{w:8,h:6}, dataPaths:[], priority:'P1' },
    ...
    { id:'W25', type:'evidence-board',title:'作战态势',     category:'decision', tier:'tick',   defaultSize:{w:12,h:5},  dataPaths:['pnl_live','trade_tickets','sentiment','iwencai','rule_state','live_index'], priority:'P0' },
  ];
